...
class Lstm(object):
    ...
    def __init__(self,word_embedding,idiom_embedding,settings):
        ...
        with tf.variable_scope('NCF_layer'):
            self.doc=tf.reshape(tf.tile(self.doc,[1,self.idiom_num,1]),[-1,2*self.hidden_size]) #[batchsize*idiom_num, 2 * hidden_size]
            self.idiom=tf.reshape(self.idiom,[-1,2*self.hidden_size]) #[batchsize*idiom_num, 2 * hidden_size]

            out=tf.reshape(self.ncf(self.doc,self.idiom),[-1,self.idiom_num])
            self.pred = tf.argmax(out, -1)

        with tf.name_scope('loss'):
            self.loss = tf.reduce_mean(
               tf.nn.softmax_cross_entropy_with_logits(logits=out, labels=self.label_inputs))
  ...
  # Weighted sum
    ...
    def ncf(self, x1, x2):
        #gmf
        gmf=tf.multiply(x1,x2)

        #mlp
        w1=self.weight_variable([self.hidden_size*2*2, self.hidden_size*2], name='w1')
        b1=self.bias_variable([self.hidden_size*2],name='b1')
        mlp=tf.nn.relu(tf.matmul(tf.concat([x1,x2],axis=1),w1)+b1)
        w2=self.weight_variable([self.hidden_size*2, self.hidden_size*2], name='w2')
        b2=self.bias_variable([self.hidden_size*2],name='b2')
        mlp=tf.matmul(mlp,w2)+b2

        #fusion
        w3=self.weight_variable([self.hidden_size*2*2, 1], name='w3')
        b3=self.bias_variable([1],name='b3')

        out=tf.matmul(tf.concat([mlp,gmf],axis=1),w3)+b3
