...
class Lstm(object):
    ...
    def __init__(self,word_embedding,idiom_embedding,settings):
        ...
        with tf.variable_scope('Out_layer'):
            match_matrix = tf.reshape(tf.matmul(self.idiom, tf.transpose(self.doc, [0, 2, 1])),[-1,self.doc_len]) # [batchsize*idiom_num,doc_len]
            w1=self.weight_variable([self.doc_len, 100], name='w1')
            b1=self.bias_variable([100],name='b1')
            match_matrix=tf.nn.relu(tf.matmul(match_matrix,w1)+b1)
            w2=self.weight_variable([100, 1], name='w2')
            b2=self.bias_variable([1],name='b2')
            match_matrix=tf.reshape(tf.matmul(match_matrix,w2)+b2,[-1,self.idiom_num])
            self.pred = tf.argmax(match_matrix, -1)
        with tf.name_scope('loss'):
            self.loss = tf.reduce_mean(
               tf.nn.softmax_cross_entropy_with_logits(logits=match_matrix, labels=self.label_inputs))
